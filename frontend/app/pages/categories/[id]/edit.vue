<template>
  <div class="min-h-screen bg-white p-8 font-sans text-black">
    <div class="max-w-2xl mx-auto rounded-xl border border-gray-200 bg-white shadow-sm p-8">
      <h1 class="text-2xl font-bold mb-6">Редагування категорії #{{ route.params.id }}</h1>
...
const loadCategory = async () => {
  try {
    const data = await $fetch<any>(`/api/admin/blog/categories/${route.params.id}`)
    state.value = {
      title: data.title || '',
      slug: data.slug || '',
...
const onSubmit = async (event: FormSubmitEvent<Schema>) => {
  try {
    await $fetch(`/api/admin/blog/categories/${route.params.id}`, {
      method: 'PUT',
      body: event.data
    })
    router.push('/categories')
  } catch (e: any) {
    alert('Помилка при збереженні: ' + (e.data?.message || e.message))
  }
}
</script>
